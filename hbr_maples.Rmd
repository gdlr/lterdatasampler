...
```{r}
hbr_maples_reference <- hbr_maples %>% 
...
ref <- ggplot() +
  geom_point(data = hbr_maples_reference, aes(x = stem_length, y = stem_dry_mass, color = factor(year)), size = 2, shape = 21) +
  labs(x = NULL, y = "Stem Dry Mass (g)",
    subtitle = "Reference",
    color = NULL
  )

w1 <- ggplot() +
  geom_point(data = hbr_maples_w1, aes(x = stem_length, y = stem_dry_mass, color = factor(year)), size = 2, shape =21) +
  labs(x = NULL, y = NULL,
    subtitle = "Calcium added",
    color = NULL)

hbr <- ref + w1 + patchwork::plot_layout(guides = "collect") + 
  plot_annotation(title = "Stem Dry Mass vs. Stem Length in Sugar Maple Seedlings \nin Reference and Calcium Added watersheds", subtitle = "Hubbard Brook LTER") & 
  theme_minimal() & 
  scale_color_manual(values = c("#fff1b7", "#eae5eb")) &
  theme(plot.background = element_rect(fill = "#697251", color = "#697251"),
        plot.title = element_text(color = "#eff2e7", face = "bold"),
        text = element_text(color = "#c7d2ab"),
        axis.text = element_text(color = "#23261b"),
        panel.grid = element_line(color = "#343928")) & 
  scale_y_continuous(limits = c(0, 0.16)) &
  scale_x_continuous(limits = c(50, 130))

hbr

ggsave(plot = hbr, filename = "hbr_maples.png", height = 4, width = 7)
ggsave(plot = hbr, filename = "hbr_maples.pdf", height = 4, width = 7)

```

```{r}
ref + w1 + patchwork::plot_layout(guides = "collect") &
  theme_void() &
  scale_color_manual(values = c("#fff1b7", "#eae5eb")) &
